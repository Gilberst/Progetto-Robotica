...
class DDS(threading.Thread):
    ...
    def subscribe(self, _varlist):
        ...
        self.socket.sendto(data.getvalue(), (self.remote_host, self.remote_port))
    ...
    def run(self):
        ...
        while self.__running:
            selected=select.select([self.socket],[],[], 0.5)

            if selected==[]:
                continue

            _data=self.socket.recvfrom(1024)
