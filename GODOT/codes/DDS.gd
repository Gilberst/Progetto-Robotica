...
const TIME_TO_LIVE = 3   # 3 seconds
...
func _process(delta: float) -> void:
	...
	for peer in udp_peers:
		while peer.get_available_packet_count() > 0:
			var collection : SubscribedVarCollection
			if subscribers.get(peer) == null:
				collection = SubscribedVarCollection.new()
				subscribers[peer] = collection
			else:
				collection = subscribers[peer]
